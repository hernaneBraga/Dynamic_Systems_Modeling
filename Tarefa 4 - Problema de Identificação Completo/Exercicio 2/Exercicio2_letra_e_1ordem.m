...
Psi1=[y1(1:N-2) u1(2:N-1)];
params = pinv(Psi1)*y1(3:end);

%% Validando a resposta encontrada
Hz_limpo = tf(params(2), [1 -params(1)], Ts);
...
u2 = ones(1, N);
yreal = step(Hz,t);
y1ordem = step(Hz_limpo,t);

% =============== SIMULACAO PASSO A FRENTE ======================
y_passo_frente = validacao_passo_frente(u2, yreal, 1, params);
y_passo_frente = y_passo_frente';

rmse_passo_frente = RMSE(yreal, y_passo_frente);
disp('RMSE simulacao de 1º ordem um passo a frente sem ruído:'); 
disp(rmse_passo_frente);


% ===============  SIMULACAO LIVRE ======================
y_livre = validacao_livre_1ordem(u2, yreal, params);

rmse_livre = RMSE(yreal(1:end-3), y_livre);
disp('RMSE simulacao livre sem ruído:'); 
disp(rmse_livre);
...
xlabel('Atraso');
title('FAC resíduos de um passo a frente - 1º ordem (sem ruído)');

subplot(212)
myccf2((yreal(4:end) - y_livre), length(y_livre), 0, 1,'k');
xlabel('Atraso');
title('FAC resíduos da simulacao livre - 1º ordem (sem ruído)');
...
Psi1_ruido=[ym(2:N-1) u1(1:N-2)];
params_ruido = pinv(Psi1_ruido)*ym(3:end);

Hz_ruido = tf(params_ruido(2), [1 -params_ruido(1)], Ts);


disp(['Parametros encontrados de 1º ordem com Sinal/Ruído = ' num2str(S_R) 'db']);
disp(params_ruido);
...
y_passo_frente_ruido = validacao_passo_frente(u2, ym_real, 1, params_ruido);
...
y_livre_ruido = validacao_livre_1ordem(u2, ym_real, params_ruido);
...
title('Validação 1º ordem um passo a frente: Resposta ao degrau');
xlabel('Amostras');
...
title('Validação 1º ordem simulação livre: Resposta ao degrau');
xlabel('Amostras');
leg = legend({'y','$\hat{y}$ sem ruido','$\hat{y}$ 25db'});
set(leg, 'Interpreter', 'latex');
...
hold on
pzmap(Hz_ruido, 'r')
title('Diagrama de polos e zeros: 1º e 2º ordem');
